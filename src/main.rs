...
use let_engine::{networking::*, prelude::*};
...
fn main() {
    ...
    let mut engine = Engine::<Game, u32>::new(
...
fn main() {
    ...
    let server = engine.new_public_server(7774).unwrap();

    // Initialize the game struct after the engine was initialized.
    let game = Game::new(server);
...
struct Game {
    ...
    exit: bool,
    #[cfg(feature = "client")]
    client: GameClient<u32>,
    #[cfg(not(feature = "client"))]
    server: GameServer<u32>,
...
impl Game {
    pub fn new(
        #[cfg(feature = "client")] client: GameClient<u32>,
        #[cfg(not(feature = "client"))] server: GameServer<u32>,
    ) -> Self {
        ...
        let font = Font::from_slice(include_bytes!("../assets/Px437_CL_Stingray_8x16.ttf"))
...
impl let_engine::Game<u32> for Game {
    async fn start(&mut self) {
        self.server.start();
    }
    #[cfg(feature = "client")]
    async fn net_event(&mut self, addr: std::net::SocketAddr, message: RemoteMessage<u32>) {}
    #[cfg(not(feature = "client"))]
    async fn net_event(&mut self, addr: std::net::SocketAddr, message: RemoteMessage<u32>) {}
...
impl Paddle {
    pub fn new(
        layer: &Arc<Layer>,
        #[cfg(feature = "client")] controls: (Key, Key),
        x: f32,
    ) -> Self {
        let height = 0.05;
